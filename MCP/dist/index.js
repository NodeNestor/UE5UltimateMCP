...
import { z } from "zod";
import { UE_PORT, UE_BASE_URL, getUEHealth, ensureUE, uePost, gracefulShutdown, log, } from "./ue-bridge.js";
import { classifyTool, getCategoryForTool, categorizeTool, convertToMCPSchema, CATEGORIES, } from "./tool-router.js";
import { getContextForTool, getContextForQuery, loadContextForCategory, listCategories, getCategoryInfo, listAvailableContextFiles, } from "./context-loader.js";
import { formatToolResponse, safeStringify } from "./helpers.js";
// ---------------------------------------------------------------------------
// Configuration
// ---------------------------------------------------------------------------
...
async function fetchToolList() {
    ...
    try {
        ...
        log.error("Error fetching tool list", { error: error.message });
        return toolCache.tools;
    }
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
server.tool("server_status", "Check UE5 connection status, tool count, and server health", {}, async () => {
            ...
            content: [
                {
                    ...
                    text: safeStringify({
                        connected: false,
                        port: UE_PORT,
                        message: "UE5 server is not running. Start Unreal Editor with UE5UltimateMCP plugin enabled.",
                    }),
                },
            ],
            isError: true,
...
// ---------------------------------------------------------------------------
server.tool("get_ue_context", `Load UE5 API context documentation. Categories: ${listCategories().join(", ")}`, {
...
async function registerDynamicTools() {
    ...
    for (const tool of tools) {
        ...
        if (tier === "simple") {
            ...
            server.tool(tool.name, tool.description, zodSchema, async (args) => {
                ...
                return formatToolResponse(tool.name, result, contextFn);
            });
        }
    }
    ...
    for (const tool of tools) {
        if (classifyTool(tool.name) !== "mega")
            continue;
        const catKey = getCategoryForTool(tool.name);
        if (!catKey || registeredCategories.has(catKey))
            continue;
        ...
        server.tool(`ue_${catKey}`, `${cat.label}: ${cat.description}. Operations: ${toolNames.join(", ")}`, {
            ...
            const result = await executeTool(operation, (opParams || {}));
            const contextFn = INJECT_CONTEXT ? getContextForTool : undefined;
            return formatToolResponse(operation, result, contextFn);
        });
    }
    // Register hidden tools (callable but won't clutter the list)
    for (const tool of tools) {
        if (classifyTool(tool.name) !== "hidden")
            continue;
        server.tool(tool.name, tool.description, { params: z.record(z.unknown()).optional().describe("Tool parameters") }, async (args) => {
            const result = await executeTool(tool.name, (args.params || {}));
            return formatToolResponse(tool.name, result);
        });
    }
 ...
 */
function buildZodFromJsonSchema(jsonSchema) {
