...
// ---------------------------------------------------------------------------
// Category mapping for mega-tools
// ---------------------------------------------------------------------------
...
export function classifyTool(toolName: string): ToolTier {
  for (const pat of SIMPLE_PATTERNS) {
    if (pat.test(toolName)) return "simple";
  }
  for (const pat of HIDDEN_PATTERNS) {
    if (pat.test(toolName)) return "hidden";
  }
  return "mega";
}
...
export function getCategoryForTool(toolName: string): string | null {
  for (const [key, cat] of Object.entries(CATEGORIES)) {
    for (const pat of cat.toolPatterns) {
      if (pat.test(toolName)) return key;
    }
  }
  return null;
}
...
export function convertToMCPSchema(
  params: UEToolParam[] | undefined,
  compact = false,
...
export function convertAnnotations(
  annotations?: UETool["annotations"],
 ...
 */
export function buildCategoryRouterSchema(
  categoryKey: string,
  tools: UETool[],
